...
### Added
- **Quality tooling: `cargo-deny` + `cargo-machete` + `taplo`.** Three new pieces of supply-chain and formatting hygiene, all wired into `just check` and a new `quality` CI job (runs once on Ubuntu via `taiki-e/install-action`, no per-OS duplication). `cargo-deny check` enforces a permissive-license allowlist on every transitive dep, fails on RustSec advisories or yanked crates, and denies git/unknown-registry sources; config is in [`deny.toml`](deny.toml) (~50 lines, dev-deps excluded). `cargo-machete` flags unused dependencies on every PR. `taplo fmt --check` keeps `Cargo.toml` and `deny.toml` consistently formatted. The trade-off documented in [CONTRIBUTING.md](CONTRIBUTING.md): `just check` now needs network for the advisory-DB fetch; everything else still runs offline.
...
- New [`tests/http.rs`](tests/http.rs) — stands up a `tiny_http` server on an ephemeral localhost port, serves the bundled `jtr-index/` verbatim, and runs the real CLI against it. The first integration test to exercise the production HTTP fetch path (every other test uses `file://`). Asserts both `/index.json` and `/recipes/postgres-dev.json` are requested, which proves relative-URL resolution against an absolute base URL works end-to-end. Dev-dep: `tiny_http`.
